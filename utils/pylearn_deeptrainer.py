...
def get_layer_trainer_logistic(layer, trainset, epochs):
    # configs on sgd
...
def get_layer_trainer_sgd_autoencoder(layer, trainset, epochs):
    # configs on sgd
    train_algo = SGD(
            learning_rate = 0.1,
...
def trainNetwork(structure = [[n_input, 100], [100, 500], [500, 1000], [1000, n_output]], unsupervised_epochs, supervised_epochs):
    DenseDesignMatrix(X=data, y=self.y, y_labels=2)
    ...
    layer_trainers = []
    layer_trainers.append(get_layer_trainer_sgd_rbm(layers[0], trainset[0]), unsupervised_epochs)
    layer_trainers.append(get_layer_trainer_sgd_autoencoder(layers[1], trainset[1]),  unsupervised_epochs)
    layer_trainers.append(get_layer_trainer_sgd_autoencoder(layers[2], trainset[2]),  unsupervised_epochs)
    layer_trainers.append(get_layer_trainer_logistic(layers[3], trainset[3]), supervised_epochs)
