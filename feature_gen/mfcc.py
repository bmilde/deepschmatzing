...
#             print self.filters[leftfr:rightfr,whichfilt]
    ...
    def sig2logspec(self, sig):
        ...
        for fr in xrange(nfr):
            #if(fr-50 % 200 == 0):
            #    print 'sig2logspec: at ', fr, nfr, (float(fr) / float(nfr)) * 100, '%'
            start = round(fr * self.fshift)
            end = min(len(sig), start + self.wlen)
            ...
            self.framepos += [(start, end)]
        #print 'Sig2logspec: done, proccesed ', nfr ,' frames.'
        return mfcc
    ...
    def logspec2s2mfc(self, frames):
        return numpy.array([numpy.dot(logspec, self.s2dct.T) / self.nfilt for logspec in frames])
